 ...
 * Purpose: Display talent demand insights from backend
 */
...
export default function TalentDemandCard({ data }) {
  if (!data) {
    return (
      <div className="card" style={{ padding: 20, textAlign: "center" }}>
        ...
        <div className="skeleton" style={{ height: 40, width: "40%", margin: "0 auto" }} />
      </div>
    );
  }
  ...
  const items = [
    { 
      label: "Most Sought-After Role", 
      value: data.mostSoughtAfterRole || "N/A", 
      icon: <Target size={18} />, 
      color: "#F59E0B" 
    },
    { 
      label: "Fastest Growing Category", 
      value: data.fastestGrowingCategory || "N/A", 
      icon: <TrendingUp size={18} />, 
      color: "#10B981" 
    },
    { 
      label: "Total Active Students", 
      value: data.totalActiveStudents?.toLocaleString() || 0, 
      icon: <Users size={18} />, 
      color: "#3B82F6" 
    },
    { 
      label: "Total Active Jobs", 
      value: data.totalActiveJobs?.toLocaleString() || 0, 
      icon: <Briefcase size={18} />, 
      color: "#8B5CF6" 
    },
    { 
      label: "Avg Applicants per Job", 
      value: data.averageApplicantsPerJob?.toFixed(1) || "0.0", 
      icon: <TrendingUp size={18} />, 
      color: "#EC4899" 
    },
    { 
      label: "Student-to-Job Ratio", 
      value: data.studentToJobRatio?.toFixed(1) || "0.0", 
      icon: <Users size={18} />, 
      color: "#14B8A6" 
    },
  ];
